...
export class ClustService {
  ...
  url = 'http://localhost:3000/clusts'; // api
  ...
  getCarById(id: number): Observable<SubGrupoModel> {
    return this.httpClient.get<SubGrupoModel>(this.url + '/' + id)
      .pipe(
        retry(2),
        catchError(this.handleError)
      );
  ...
  updateClusts(Clusts: SubGrupoModel): Observable<SubGrupoModel> {
    return this.httpClient.put<SubGrupoModel>(this.url + '/' + Clusts.id, JSON.stringify(Clusts), this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.handleError)
      );
  ...
  deleteClusts(Clusts: SubGrupoModel) {
    return this.httpClient.delete<SubGrupoModel>(this.url + '/' + Clusts.id, this.httpOptions)
      .pipe(
        retry(1),
        catchError(this.handleError)
      )
